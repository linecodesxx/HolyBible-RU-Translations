...
`Пункт 5` исправляет баг, который выявляется после парсинга. Суть в том, что парсер первые три `JSON-объекта` заполняет текстом со всей страницы, `cleaner.py` очищает и сохраняет новый файл с префиксом `cleaned`.
